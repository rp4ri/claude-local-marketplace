...
Use Claude Code's native tools to verify:

1. **WebSearch** to find official docs: `WebSearch("<library> <specific behavior> site:<official-docs-domain>")`
2. **WebFetch** to read the doc page: `WebFetch(url: "<docs-url-from-search>")`
3. **Perplexity fallback**: `mcp__perplexity-ask__perplexity_ask(question: "<specific claim to verify>")`

Use sparingly — only when a finding hinges on library behavior you aren't sure about. If all lookups fail, note "could not verify from docs" in the evidence field.
...
For analysis methodology and calibration examples (3 confirmed findings + 2 false positives with STRIDE/CWE), read `${CLAUDE_PLUGIN_ROOT}/skills/dev/examples/hunter-examples.md` before starting your scan.
