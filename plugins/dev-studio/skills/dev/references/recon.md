...
When you need to verify framework behavior or library defaults during reconnaissance, use Claude Code's native tools:

1. **WebSearch** to find official docs: `WebSearch("<library> <specific question> site:<official-docs-domain>")`
...
If all lookups fail, note "could not verify from docs" and proceed based on code analysis.
