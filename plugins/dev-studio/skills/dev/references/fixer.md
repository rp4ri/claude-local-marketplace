...
Use Claude Code's native tools to verify the correct API pattern:

1. **WebSearch** to find official docs: `WebSearch("<library> <correct API pattern> site:<official-docs-domain>")`
2. **WebFetch** to read the doc page: `WebFetch(url: "<docs-url-from-search>")`
3. **Perplexity fallback**: `mcp__perplexity-ask__perplexity_ask(question: "<specific API usage question>")`
