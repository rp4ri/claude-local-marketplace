...
```
node "${CLAUDE_PLUGIN_ROOT}/scripts/fix-lock.cjs" acquire ".bug-hunter/fix.lock" $DYNAMIC_TTL
```
...
```
node "${CLAUDE_PLUGIN_ROOT}/scripts/fix-lock.cjs" renew ".bug-hunter/fix.lock" "$LOCK_OWNER_TOKEN"
```
...
```
node "${CLAUDE_PLUGIN_ROOT}/scripts/fix-lock.cjs" release ".bug-hunter/fix.lock" "$LOCK_OWNER_TOKEN"
```
...
```bash
node "${CLAUDE_PLUGIN_ROOT}/scripts/schema-validate.cjs" fix-report ".bug-hunter/fix-report.json"
```

Render the Markdown companion when humans need it:

```bash
node "${CLAUDE_PLUGIN_ROOT}/scripts/render-report.cjs" fix-report ".bug-hunter/fix-report.json" > ".bug-hunter/fix-report.md"
```
