...
1. Read `${CLAUDE_PLUGIN_ROOT}/prompts/recon.md` with the Read tool — do NOT act from memory.
...
1. Read `${CLAUDE_PLUGIN_ROOT}/skills/dev/references/hunter.md` with the Read tool.
2. Read `${CLAUDE_PLUGIN_ROOT}/skills/dev/references/doc-lookup.md` with the Read tool.
...
6. Validate the artifact immediately:
   ```bash
   node "${CLAUDE_PLUGIN_ROOT}/scripts/schema-validate.cjs" findings ".bug-hunter/findings.json"
   ```
...
1. Initialize state:
   ```bash
   node "${CLAUDE_PLUGIN_ROOT}/scripts/bug-hunter-state.cjs" init ".bug-hunter/state.json" "local-sequential" ".bug-hunter/source-files.json" 30
   ```
2. For each chunk:
   a. Get next chunk:
      ```bash
      node "${CLAUDE_PLUGIN_ROOT}/scripts/bug-hunter-state.cjs" next-chunk ".bug-hunter/state.json"
      ```
   b. Mark in-progress:
      ```bash
      node "${CLAUDE_PLUGIN_ROOT}/scripts/bug-hunter-state.cjs" mark-chunk ".bug-hunter/state.json" "<chunk-id>" in_progress
      ```
   ...
   e. Record findings in state:
      ```bash
      node "${CLAUDE_PLUGIN_ROOT}/scripts/bug-hunter-state.cjs" record-findings ".bug-hunter/state.json" ".bug-hunter/chunk-<id>-findings.json" "local-sequential"
      ```
   f. Mark done:
      ```bash
      node "${CLAUDE_PLUGIN_ROOT}/scripts/bug-hunter-state.cjs" mark-chunk ".bug-hunter/state.json" "<chunk-id>" done
      ```
...
1. Read `${CLAUDE_PLUGIN_ROOT}/skills/dev/references/skeptic.md` with the Read tool.
2. Read `${CLAUDE_PLUGIN_ROOT}/skills/dev/references/doc-lookup.md` with the Read tool.
...
7. Validate it immediately:
   ```bash
   node "${CLAUDE_PLUGIN_ROOT}/scripts/schema-validate.cjs" skeptic ".bug-hunter/skeptic.json"
   ```
...
1. Read `${CLAUDE_PLUGIN_ROOT}/prompts/referee.md` with the Read tool.
...
7. Validate them immediately:
   ```bash
   node "${CLAUDE_PLUGIN_ROOT}/scripts/schema-validate.cjs" referee ".bug-hunter/referee.json"
   ```
